...
function getSign(t) {
  ...
  return (
    ...
    ''.concat((b %= 1e6).toString(), '.').concat((b ^ f) as any)
  );
}
...
const getToken = async () => {
  const htmRes: any = await request({
...
function getAudioURI(text, lang) {
  return root + ('/gettts?lan=' + lang + '&text=' + encodeURIComponent(text) + '&spd=3&source=web');
}
...
function getError(code: string, msg?: string) {
  var e = new Error(msg) as any;
  e.code = code;
  return e;
}
