...
import { hasChinese, diffDses, getRelatedBos, CONFIG_FILE, lookForFiles } from './utils';
...
import { LocalDsManager } from './LocalDsManager';
...
export class Manager {
  ...
  async initRemoteDataSource(config = this.currConfig) {
    ...
    // 只查询当前数据源，用户只关心当前数据源。
    let oldRemoteSource = LocalDsManager.getLatestDsInProject(this.projectRoot, currProj);
    ...
    if (oldRemoteSource) {
      ...
      await LocalDsManager.saveDataSource(this.projectRoot, currProj, this.remoteDataSource);
    }
  }
  ...
  async readRemoteDataSource(config = this.currConfig) {
    ...
    if (!oldRemoteSource) {
      if (this.remoteDataSource) {
        LocalDsManager.saveDataSource(this.projectRoot, currProj, this.remoteDataSource);
  ...
  setFilesManager() {
    ...
    const { default: Generator, FileStructures: MyFileStructures } = getTemplate(this.projectRoot, {
  ...
  getReportData() {
    ...
    return LocalDsManager.getReportData(this.projectRoot, currProj);
  }
  ...
  openReport() {
    ...
    LocalDsManager.openReport(this.projectRoot, currProj);
  }
