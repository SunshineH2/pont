...
/** 内部方法，递归扫描文件夹 */
function readDirRecursively(dirPath: fs.PathLike, callback: (path: fs.PathLike) => void) {
...
export function main(manager: Manager) {
  ...
  fs.writeFileSync(
    outputFileName,
    JSON.stringify(
      [...unusedRequests].map(({ method, path, description, originName, modName, name }) => ({
        method,
        path,
        description,
        originName,
